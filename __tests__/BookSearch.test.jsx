  ...
  describe("searching by each field alone", () => {
    ...
    describe("modifying the country field", () => {
      ...
      it("should filter books on the country query", () => {
        ...
        expect(within(bookEls[0]).queryByText(/\bDante Alighieri\b/)).toBeInTheDocument();
        expect(within(bookEls[1]).queryByText(/\bGiovanni Boccaccio\b/)).toBeInTheDocument();
        expect(within(bookEls[2]).queryByText(/\bGiacomo Leopardi\b/)).toBeInTheDocument();
        expect(within(bookEls[3]).queryByText(/\bElsa Morante\b/)).toBeInTheDocument();
        expect(within(bookEls[4]).queryByText(/\bItalo Svevo\b/)).toBeInTheDocument();
      });
    });
    ...
    describe("modifying the language field", () => {
      ...
      it("should filter books on the language query", () => {
        ...
        expect(within(bookEls[0]).queryByText(/\bJorge Luis Borges\b/)).toBeInTheDocument();
        expect(within(bookEls[1]).queryByText(/\bMiguel de Cervantes\b/)).toBeInTheDocument();
        expect(within(bookEls[2]).queryByText(/\bGypsy Ballads\b/)).toBeInTheDocument();
        expect(within(bookEls[3]).queryByText(/\bOne Hundred Years of Solitude\b/)).toBeInTheDocument();
        expect(within(bookEls[4]).queryByText(/\bLove in the Time of Cholera\b/)).toBeInTheDocument();
        expect(within(bookEls[5]).queryByText(/\bJuan Rulfo\b/)).toBeInTheDocument();
      });
    });
  ...
  describe("filtering on multiple terms", () => {
    ...
    describe("filtering on all fields with trim", () => {
      ...
      it("should filter books on all fields", () => {
        ...
        const [book] = bookEls;
        expect(within(book).queryByText(/\bEdgar Allan Poe\b/)).toBeInTheDocument();
        expect(within(book).queryByText(/\bUnited States\b/)).toBeInTheDocument();
        expect(within(book).queryByText(/\bEnglish\b/)).toBeInTheDocument();
        expect(within(book).queryByText(/\b842\b/)).toBeInTheDocument();
        expect(within(book).queryByText(/\bTales\b/)).toBeInTheDocument();
        expect(within(book).queryByText(/\b1950\b/)).toBeInTheDocument();
      });
    });
